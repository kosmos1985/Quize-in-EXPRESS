function gameRouts(app)
{
    let goodAnswers = 0;
    let isGameOver = false;
    let callToAFrendUsed = false;
    let questionToTheCrowdUsed = false;
    let halfOnHalfUsed = false;

    const questions = [
        {
            question: 'Jaki jest najlepszy język programowania wd mnie?',
    ...
    app.get('/help/friend', (req, res) =>
{
    ...
    callToAFrendUsed = true;
    
    const doesFriendKnowAnswer = Math.random() < 0.5;
    const question = questions[goodAnswers];
    res.json({
        text: doesFriendKnowAnswer? `Wydaje mi się, że odpowieź to ${question.answers[question.correctAnswer]}`:'Hhhhh, no nie wiem...'
    })
    });
